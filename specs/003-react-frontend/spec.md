# Feature Specification: Frontend Web de Gestão de Despesas Compartilhadas (SPA)
...
2. **Given** um grupo ativo, **When** o responsável envia convites por link ou e-mail, **Then** os convites aparecem com estado rastreável e validade.
...
- O processo de build do frontend gera artefatos estáticos (HTML, CSS, JavaScript) que podem ser servidos por middleware de arquivos estáticos do servidor da API sem nenhuma configuração de servidor web adicional.
- O servidor da API é capaz de configurar roteamento para que todas as rotas não reconhecidas retornem o ponto de entrada da interface (suporte a SPA fallback).
